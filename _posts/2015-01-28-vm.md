...
updated: 2015-01-29 21:44:02 +0100
categories: Python
disqus: true
tags: Python
---
...
So, let's start writing a simple class for a stack. (*Update*: I've uploaded
and [expanded on this code on github][code]).
...
[Constant folding][constant folding] looks for obvious pieces of code that can
be precomputed at compile time. For instance, mathematical expressions
...
find one, you need to record the nameto insert the name along with an address
...
You could try to implement a [call stack][call stack] with [stack frames][stack
frames] and establish a calling convention.
